...
def test_fisher_and_taylor_rows():
    ...
    assert np.isclose(system["Gamma_0"][row_fisher, idx["i_nom_hat"]], -1.0)
    assert np.isclose(system["Gamma_f"][row_fisher, idx["pi_hat"]], -1.0)

    row_taylor = system["equation_labels"].index("taylor_rule")
    assert np.isclose(system["Gamma_l"][row_taylor, idx["i_nom_hat"]], -params["rho_i"])
    assert np.isclose(system["Gamma_0"][row_taylor, idx["pi_hat"]], -(1.0 - params["rho_i"]) * params["phi_pi"])
    assert np.isclose(system["Gamma_0"][row_taylor, idx["y_hat"]], -(1.0 - params["rho_i"]) * params["phi_y"])
