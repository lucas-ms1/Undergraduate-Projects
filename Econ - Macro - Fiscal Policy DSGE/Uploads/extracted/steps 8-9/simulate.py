...
The model is assumed to produce log-deviations from steady state directly,
so no detrending is applied here.  HP-filtering lives in moments.py and is
used only when comparing to empirical FRED moments.

Observable indices (into the state vector x) must be supplied by the caller
via the `obs_index` dict, e.g.
    obs_index = {"y": 0, "c": 1, "i": 2, "l": 3, "pi": 4}
"""
...
DEFAULT_OBS_INDEX: Dict[str, int] = {
    "y":  0,   # log output deviation
    "c":  1,   # log consumption deviation
    "i":  2,   # log investment deviation
    "l":  3,   # log hours deviation
    "pi": 4,   # log inflation deviation
}
...
def run_simulation(
    ...
    """
    X = simulate(A, B, T=T, seed=seed, x0=x0)
