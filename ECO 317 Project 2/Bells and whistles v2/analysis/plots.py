...
def make_impulse_like_panel(base_path: np.ndarray, alt_path: np.ndarray, title: str, layout_factory, style_axes):
    """Simple deterministic response panel: baseline vs alternative + difference."""
    t = np.arange(len(base_path))
    fig = make_subplots(rows=1, cols=2, subplot_titles=("Levels", "Alternative - Baseline"))
    fig.add_trace(go.Scatter(x=t, y=base_path, mode="lines", name="Baseline"), row=1, col=1)
    fig.add_trace(go.Scatter(x=t, y=alt_path, mode="lines", name="Alternative"), row=1, col=1)
    fig.add_trace(go.Bar(x=t, y=alt_path - base_path, name="Delta"), row=1, col=2)
    fig.update_layout(**layout_factory(title=title, height=420))
    return style_axes(fig)
