...
def welfare_cost_of_volatility(base_result: dict, low_vol_result: dict, shock_labels: list[str]) -> pd.DataFrame:
    """Compute welfare gap from lower-volatility counterfactual."""
    base_v = np.asarray(base_result["value_function"], dtype=float)
    low_v = np.asarray(low_vol_result["value_function"], dtype=float)
    if base_v.ndim == 1:
        base_v = base_v[np.newaxis, :]
        low_v = low_v[np.newaxis, :]
    rows = []
    for j, label in enumerate(shock_labels):
        diff = low_v[:, j] - base_v[:, j]
        rows.append(
            {
                "shock_state": label,
                "mean_delta_value_low_minus_base": float(np.nanmean(diff)),
                "p10_delta_value": float(np.nanpercentile(diff, 10)),
                "p90_delta_value": float(np.nanpercentile(diff, 90)),
            }
        )
    return pd.DataFrame(rows)
